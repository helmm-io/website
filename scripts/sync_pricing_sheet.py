...
6. Once you've picked the tier: match every remaining line item on the invoice to the closest HELM equivalent, applying the bundling rules above. Include that tier's support fee and infrastructure fee. Use exact HELM sale prices only. monthly_price = per-unit price only, never a line total. helm_total = sum of all (qty x monthly_price) for the chosen tier, including any paid add-ons you added, and excluding anything bundled free at that tier. Exclude unmatched items from total and list them in unmatched_items.
7. In notes, always name the tier you chose and briefly explain why in plain English the sales rep can read aloud, especially when you picked a cheaper tier than headcount alone would suggest (say what it saves versus the headcount implied tier), or when you stuck with a pricier tier despite a cheaper option (say why). Also mention anything bundled in for free.
