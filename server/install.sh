#!/bin/bash
# Idempotent server setup for the HubSpot + Claude proxies and nginx API routing.
...
install -m 755 "$SRC/helm-claude-proxy-start" /usr/local/bin/helm-claude-proxy-start
install -m 644 "$SRC/helm-claude-proxy.service" /etc/systemd/system/helm-claude-proxy.service
...
systemctl enable helm-claude-proxy >/dev/null 2>&1 || true
systemctl restart helm-claude-proxy
